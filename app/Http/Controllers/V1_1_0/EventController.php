...
class EventController extends Controller
{
    ...
    public function __construct(Event $event, UserBookmark $userBookmark, NumberView $numberView, ClientInterest $clientInterest)
    {
    ...
    public function search(Request $request)
    {
        ...
        try {
            ...
            $event = $event->with(['eventImages' => function ($query){
                $query->select('eventId', DB::raw("CONCAT(path, fileName) as fullPath"));
            },'organizer'])->where(function ($query) use ($data) {
        ...
        try {
            $data->map(function ($item) {
                ...
                if(auth($this->guard)->user()){
                    $item->isBookMarked = $this->clientInterest->where('user_id', auth($this->guard)->user()->id)->whereTableType('events')->where('table_id', $item->id)->exists();
                }
                else{
            ...
            if(auth($this->guard)->user()) {
                $data->isBookMarked = $this->clientInterest->where('user_id', auth($this->guard)->user()->id)->whereTableType('events')->where('table_id', $data->id)->exists();
            } else {
    ...
    public function addQuestCount(Request $request){
        $data = $request->all();

        $this->numberView->storeAsQuests($data['id']);
