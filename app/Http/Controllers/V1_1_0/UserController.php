...
class UserController extends Controller
{
    ...
    {
        // $data = $request->all();
        $response = User::getModel()->simplePaginate(100);

        // $response = DB::table('users')->simplePaginate(100);

        // return listResponse($response);


        // $user = auth($this->guard)->user();
        return listResponse($response, 200);
    }

    public function userByLocation()
    {
        $user = auth($this->guard)->user();
        return $this->responseSuccess($user, 200);
    }
    public function userByQuest()
    {
        $user = auth($this->guard)->user();
        return $this->responseSuccess($user, 200);
    }
    public function userByGender()
    {
        $user = auth($this->guard)->user();
        return $this->responseSuccess($user, 200);
    }
