...
class UserController extends Controller {
    ...
    public function edit($uid) {
        $data = User::withTrashed()->with(['hasOneWalletAccount'])->where('code', $uid)->first();
        if($data){
	        $data->balance = $this->walletRepository->balanceInquiry($data->hasOneWalletAccount->account_number);
	        unset($data->hasOneWalletAccount);
	        return $this->responseSuccess($data, 200);
        }

        // return $data;
        return $this->responseFail(["code" => 422, "message" => "No data found"], 422);
    }
