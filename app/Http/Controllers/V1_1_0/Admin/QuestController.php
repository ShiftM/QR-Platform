...
class QuestController extends Controller
{
    public function index(Request $request){
        ...
        if(isset($request->boothId) && $request->boothId){
            $data = $data->with(['event' => function($q){
                $q->withTrashed();
            }])->where('boothId', $request->boothId);
        }
        if(isset($filter['event_id']) && $filter['event_id']){
            $data = $data->where('eventId',$filter['event_id'])->withTrashed();
        }
        
        if(isset($filter['paginate']) && $filter['paginate'] == "false"){
	        $data = $data->get();
        }
        else{
