...
class EventController extends Controller
{
    ...
    public function __construct(Event $event, EventDay $eventDay, EventSegment $eventSegment)
    {
    ...
    public function index(Request $request){
        ...
        if($request->filter){
            ...
            // $data = $data->where(\DB::raw('CONCAT_WS(" ", `name`,`title`)'), 'like', '%' . $request->filter . '%');
            
        }

        if($request->from && $request->to){
            
            $data = $data->whereBetween('startDate', [date('y-m-d H:i:s', strtotime($request->from)), 
            date('y-m-d h:i:s', strtotime($request->to))]);
        }
        
        $data = $data->orderBy('startDate', 'desc')->distinct()->paginate(25);
